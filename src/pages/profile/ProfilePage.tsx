            ...
            {isEditing ? (
              <>
                ...
                <button onClick={handleSubmit} className="btn-primary" disabled={isUploading}>
                  <Save className="h-4 w-4 mr-2" /> {isUploading ? 'Saving...' : 'Save'}
                </button>
              </>
            ) : (
          ...
          <div className="relative">
            ...
            {isEditing && (
              <div className="absolute bottom-0 right-0 flex gap-1">
                <button className="p-2 bg-primary-600 text-white rounded-full hover:bg-primary-700" onClick={() => fileInputRef.current?.click()} type="button">
                  <Camera className="h-4 w-4" />
                </button>
                {avatarFile && (
                  <button className="p-2 bg-red-500 text-white rounded-full hover:bg-red-600" onClick={handleRemoveAvatar} type="button">
                    <X className="h-4 w-4" />
                  </button>
                )}
              </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {user?.is_identity_verified && (
                <span className="badge badge-success"><Shield className="h-3 w-3 mr-1" /> Identity Verified</span>
              )}
            </div>
          </div>
        </div>
              ...
              <div key={item.id} className="flex gap-4 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors border border-transparent hover:border-gray-200 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link to={`/assets/${item.id}`} className="text-[10px] font-bold uppercase tracking-wider bg-white px-3 py-1 rounded-full border border-gray-200 group-hover:bg-primary-600 group-hover:text-white group-hover:border-primary-600 transition-all">View</Link>
                  </div>
                </div>
              </div>
            ))}
