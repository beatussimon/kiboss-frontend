...
const categoryConfig: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
  BOOKING: { icon: <Calendar className="h-5 w-5" />, color: 'text-primary-600', bg: 'bg-primary-50' },
            ...
            return (
              <div
                ...
                className={`card p-4 transition-all ${notification.status === 'READ' ? 'opacity-60' : 'border-l-4 border-l-primary-500'
                  } ${hasAction ? 'cursor-pointer hover:shadow-lg hover:bg-gray-50' : ''}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 dark:text-white truncate">{notification.title}</p>
